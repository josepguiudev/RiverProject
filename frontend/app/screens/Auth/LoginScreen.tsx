...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error de acceso",
        typeof errorMsg === "string" ? errorMsg : "Credenciales incorrectas",
      );
    } finally {
  ...
  if (isWeb) {
    return (
      <ScrollView
        contentContainerStyle={{ flexGrow: 1, backgroundColor: "#0e0d0df1" }}
      >
          ...
          <View style={[styles.caja, isDesktopView && { width: 500 }]}>
            ...
            >
              <Text style={styles.texto}>
                ¿No tienes cuenta?{" "}
                <Text style={styles.blueText}>Regístrate aquí</Text>
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        >
          <View style={[styles.alineadoPersonal, { paddingVertical: 40 }]}>
            ...
            <View style={styles.caja}>
              <Text style={[styles.mainText, { marginBottom: 25 }]}>
                Iniciar Sesión
              </Text>
