                    ...
                    <View style={[styles.cajaPrincipal, globalStyles.filas]}>
                        ...
                        <View style={[styles.contenedorFila2, styles.contenedorVertical]}>
                            ...
                            <View style={[styles.contenedorUserTittleButton, {height: "84%"}, styles.contenedorVertical]}>
                                <ScrollView style={[styles.scrollView, { width: '100%'}]} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false} bounces={true}>
                                    {usuariosEncontrados && usuariosEncontrados.map((user: any, index: number) => (
                                        <View key={user.steamid || index} style={{ padding: 15, backgroundColor: '#1b2838', borderRadius: 8, marginBottom: 15, borderWidth: 1, borderColor: '#2a475e' }}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                ...
                                                <View style={{ marginLeft: 15, flex: 1 }}>
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                        <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: user.personastate === 1 ? '#66c0f4' : '#888', marginRight: 5 }} />
                                                        <Text style={{ color: user.personastate === 1 ? '#66c0f4' : '#888', fontSize: 12 }}>{user.personastate === 1 ? 'En línea' : 'Desconectado'}</Text>
                                                    </View>
                                            ...
                                            <View style={{ marginTop: 15, borderTopWidth: 1, borderTopColor: '#2a475e', paddingTop: 10 }}>
                                                <Text style={{ color: '#ccc', fontSize: 11, marginBottom: 5 }}><Text style={{ fontWeight: 'bold' }}>País:</Text> {user.loccountrycode || 'N/A'}</Text>
                                                <Text style={{ color: '#ccc', fontSize: 11, marginBottom: 5 }}><Text style={{ fontWeight: 'bold' }}>Creado el:</Text> {new Date(user.timecreated * 1000).toLocaleDateString()}</Text>
                                                <Text style={{ color: '#66c0f4', fontSize: 11, marginTop: 5, textDecorationLine: 'underline' }} numberOfLines={1}>{user.profileurl}</Text>
                                            </View>
                                        </View>
                                    ))}
                                </ScrollView>
                            ...
                            <View style={[styles.contenedorUserTittleButton, {height: "84%"}, styles.contenedorVertical]}>
                                <ScrollView style={[styles.scrollView, { width: '100%'}]} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false} bounces={true}>
                                    {Array.isArray(juegosEncontrados) && juegosEncontrados.map((game: any, index: number) => {
                                        ...
                                        return (
                                            <View key={game.appid || index} style={{ flexDirection: 'row', alignItems: 'center', padding: 10, backgroundColor: '#1b2838', borderRadius: 6, marginBottom: 8, borderLeftWidth: 4, borderLeftColor: '#66c0f4' }}>
                                                <Image source={{ uri: iconUrl }} style={{ width: 42, height: 42, borderRadius: 4, backgroundColor: '#171a21' }} resizeMode="cover"/>
                                                <View style={{ marginLeft: 12, flex: 1 }}>
                                                    <Text style={{ color: '#ffffff', fontWeight: 'bold', fontSize: 13 }} numberOfLines={1}>{game.name}</Text>
                                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
                                                        <Text style={{ color: '#888', fontSize: 10 }}>ID: {game.appid}</Text>
                                                        {game.playtime_forever !== undefined && <Text style={{ color: '#66c0f4', fontSize: 10, fontWeight: '600' }}>{Math.floor(game.playtime_forever / 60)}h</Text>}
                                                    </View>
                                                </View>
                                            </View>
                                        );
                                    ...
                                    {juegoDetalle && Object.keys(juegoDetalle).map((key) => {
                                        ...
                                        return (
                                            <View key={key} style={{ padding: 15 }}>
                                                {gameData.header_image && <Image source={{ uri: gameData.header_image }} style={{ width: '100%', height: 120, borderRadius: 8, marginBottom: 10 }} resizeMode="cover"/>}
                                                <Text style={{ color: 'gold', fontWeight: 'bold', fontSize: 18 }}>{gameData.name}</Text>
                                                <Text style={{ color: '#888', fontSize: 12, marginBottom: 10 }}>AppID: {gameData.steam_appid} | {gameData.type.toUpperCase()}</Text>
                                                <View style={{ marginBottom: 10 }}>
                                                    ...
                                                    <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                                                        {gameData.genres && gameData.genres.map((g: any, idx: number) => (
                                                            <View key={`genre-${idx}`} style={{ backgroundColor: '#2a475e', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, marginRight: 6, marginBottom: 6 }}>
                                                ...
                                                <View style={{ marginBottom: 10 }}>
                                                    ...
                                                    <Text style={{ color: '#5dade2', fontSize: 11 }}>{gameData.categories?.map((c: any) => c.description).join(" • ")}</Text>
                                                </View>
                                                <Text style={{ color: '#aaa', fontSize: 11, fontStyle: 'italic', marginTop: 10, lineHeight: 16 }}>{gameData.short_description?.replace(/<[^>]*>?/gm, '')}</Text>
                                            </View>
                                        );
                                    })}
                                </ScrollView>
