...
const MOCK_SURVEYS = [
	{
		id_survey: 1,
		name: "Tu experiencia en el juego",
		creationDate: new Date().toISOString(),
	},
	{
		id_survey: 2,
		name: "Encuesta de hardware",
		creationDate: new Date().toISOString(),
	},
];
...
export default function ProfileScreen({ navigation, route }: any) {
	...
	const { data: profileData, isLoading: loading } = useQuery({
		...
		queryFn: async () => {
			...
			try {
				// 1. Fetch user data from DB first to get correct user info for Configuration tab
				let userDbData = null;
				if (user?.id) {
					try {
				...
				// Determinar la ID final de Steam: si está en la BD se usa, si no, se usa el fallback de pruebas
				const finalSteamId = userDbData
					? userDbData.urlIdStream || ""
					: steamId;
				...
				if (cleanSteamId && apiKey) {
					await apiFetch(
						`/api/games/sync-library?steamid=${cleanSteamId}&apiKey=${apiKey}`,
					).catch(() => {});
				}

				// 3. Load Profile via real backend API (with Mockup Fallback if not found)
				let profileJson = null;
				if (cleanSteamId) {
					try {
						const profileRes = await apiFetch(
							`/api/usersteam/by-bd-steamid/${cleanSteamId}`,
						);
						if (profileRes.ok) {
							profileJson = await profileRes.json();
						}
					} catch (e) {
						console.log("Fallo conexión perfil");
					}
				}
				// Si no está registrado en la BD o falla, usamos el mockup rico de Joaquín
				if (!profileJson) {
					profileJson = {
						personaName: userDbData?.name || "Joaquín (Steam Link)",
						steamId: "76561198166329657",
						avatarFull:
							"https://avatars.steamstatic.com/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg",
						personastate: 1, // Online
						profileUrl:
							"https://steamcommunity.com/profiles/76561198166329657",
					};
				}

				// 4. Load Top 3 Games via real backend API (with Mockup Fallback if not found)
				let gamesJson = null;
				if (cleanSteamId) {
					try {
						const gamesRes = await apiFetch(
							`/api/games/top3/${cleanSteamId}`,
						);
						if (gamesRes.ok) {
							gamesJson = await gamesRes.json();
						}
					} catch (e) {
						console.log("Fallo conexión juegos");
					}
				}
				// Si no está registrado en la BD o falla, cargamos juegos mock de su ID real
				if (!gamesJson || gamesJson.length === 0) {
					gamesJson = [
						{
							id_game: 1,
							appid: 730,
							...
							iconUrl: "icon_cs2",
							url_image:
								"https://shared.fastly.steamstatic.com/store_images/730/header.jpg",
						},
						{
							id_game: 2,
							appid: 227940,
							...
							iconUrl: "icon_hg",
							url_image:
								"https://shared.fastly.steamstatic.com/store_images/227940/header.jpg",
						},
						{
							id_game: 3,
							appid: 271590,
							...
							iconUrl: "icon_gtav",
							url_image:
								"https://shared.fastly.steamstatic.com/store_images/271590/header.jpg",
						},
					];
				}

				// 5. Load Surveys (Sigue consultando a la BD real ya que esta ruta funciona por ID de usuario)
				let surveysData = MOCK_SURVEYS;
				const userId = user?.id;
				if (userId && !isNaN(Number(userId))) {
					try {
						const surveysRes = await apiFetch(
							`/api/surveys/user/${userId}`,
						);
						if (surveysRes.ok)
							surveysData = await surveysRes.json();
					} catch (e) {
						console.log("Fallo conexión encuestas");
					}
				}

				// 6. Load Genres via real backend API (with Mockup Fallback if not found)
				let genresData = null;
				if (cleanSteamId) {
					try {
						const genresRes = await apiFetch(
							`/api/games/top-genres/${cleanSteamId}`,
						);
						if (genresRes.ok) {
							genresData = await genresRes.json();
						}
					} catch (e) {
						console.log("Fallo conexión géneros");
					}
				}
				// Si no está registrado en la BD o falla, cargamos la distribución de géneros mock
				if (!genresData || genresData.length === 0) {
					genresData = [
						{ name: "Shooter / Acción", percentage: 80 },
						{ name: "Militar / Estrategia", percentage: 15 },
						{ name: "Mundo Abierto", percentage: 5 },
					];
				}
				...
				return {
					steamProfile: {
						personaName: "Joaquín (Steam Link)",
						steamId: "76561198166329657",
						avatarFull:
							"https://avatars.steamstatic.com/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg",
						personastate: 1, // Online
	...
	const handleSettingsSave = async (payload: SavePayload) => {
		...
		if (user?.id) {
			if (user.role === "CLIENT") {
				...
				if (!updateRes.ok) {
					...
					throw new Error(
						errData.error || "Error al actualizar cliente.",
					);
				}
			} else {
		...
		if (passwordChange && user?.id) {
			...
			if (!pwRes.ok) {
				...
				throw new Error(
					errData.error || "Error al cambiar la contraseña.",
				);
			}
		}
	};
		...
		>
			...
			<View style={styles.tabBar}>
				<TouchableOpacity
					style={[
						styles.tab,
						activeTab === "perfil" && styles.tabActive,
					]}
				...
				<TouchableOpacity
					style={[
						styles.tab,
						activeTab === "configuracion" && styles.tabActive,
					]}
					...
					{user?.role === "USER" ? (
						<>
							...
							<View
								style={[
									styles.middleRow,
									isMobileView
										? styles.middleRowMobile
										: styles.middleRowDesktop,
								]}
							>
								<View
									style={
						...
						<View style={{ alignItems: "center", marginTop: 50 }}>
							...
							>
								...
								>
									Estás en tu perfil de gestión. Puedes editar
									tus datos en la pestaña de "Configuración".
								</Text>
							</View>
						</View>
					)}
